...
import { downloadFile, isObjectStorageConfigured, isStorageRef, objectKey, uploadFile } from "@/lib/storage";
...
async function runRender(projectId: string, jobId: string, baseVideoPath: string, resolution: string | null, segments: Array<{ segmentIndex: number; startTime: number; endTime: number; ayah: string; arabic: string; translation: string; overlayAssetPath: string | null }>) {
  const root = storageRoot();
  const projectDirectory = path.join(root, projectId);
  const workspace = isObjectStorageConfigured() ? await mkdtemp(path.join("/tmp", "fytobyte-render-")) : projectDirectory;
  const outputPath = path.join(workspace, "final.mp4");
  const sourcePath = isStorageRef(baseVideoPath) ? path.join(workspace, "source.mp4") : path.join(projectDirectory, "source.mp4");
  ...
  try {
    ...
    if (isStorageRef(baseVideoPath)) await downloadFile(baseVideoPath, sourcePath);
    ...
    for (const [index, segment] of segments.entries()) {
      ...
      if (segment.overlayAssetPath && isStorageRef(segment.overlayAssetPath)) await downloadFile(segment.overlayAssetPath, overlayPath);
      else await renderOverlay(segment, overlayPath, overlayDimensions(resolution));
    ...
    await renderFinal({ sourcePath, outputPath, overlayRoot: overlayDirectory, segments, onProgress: (progress) => {
    ...
    const finalRef = isObjectStorageConfigured()
      ? await uploadFile(outputPath, objectKey(projectId, "final.mp4"), "video/mp4")
      : `/uploads/${projectId}/final.mp4`;
    await db.renderJob.update({ where: { id: jobId }, data: { status: "DONE", progress: 100, stage: "Complete", outputPath: finalRef } });
  } catch (error) {
...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  void runRender(id, job.id, project.baseVideoPath, project.resolution, project.segments);
